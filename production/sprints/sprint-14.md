...
| S14-4a | 分辨率枚举 API spike | `production/sprints/sprint-14.md`（spike 报告归 production/spikes/） | Spike | 0.5d | — | DisplayServer `screen_get_resolutions()` 查证（引擎参考零覆盖——QL-STORY-READY 2026-09-19 裁决）；产出回写 docs/engine-reference/godot/（DisplayServer 小节）供 story 003 集成测试断言用 |
| S14-4 | main-menu 003 画面设置与应用/回退 | `production/epics/main-menu/story-003-settings-graphics.md` | UI | 1.0d | S14-3, S14-4a | 6 AC——分辨率枚举（API 查证前置）+应用/回退机制+全局恢复默认+720p 支持下限 |
