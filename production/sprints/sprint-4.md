...
|:--|------|:--|------|:--:|:--:|:--:|:--:|
| 0 | 多 Epic | Autoload 注册：RealmSystem #11 + SchoolSystem #19 + 6 个 Feature Autoload 顺序验证 | — | Task | 0.5d | — | Not Started |
| 1 | card-effect-engine | EffectTemplate/EffectInstance 双层对象模型（4 种子类） | `card-effect-engine/story-001-*.md` | Logic | 0.5d | — | Not Started |
| 2 | card-effect-engine | ResolutionStack 栈式结算引擎（优先级队列 + LIFO + 中断插入） | `card-effect-engine/story-002-*.md` | Logic | 0.5d | #1 | Not Started |
| 3 | card-effect-engine | 触发链硬限制 10 层 + visited_card_ids 循环检测 | `card-effect-engine/story-003-*.md` | Logic | 0.5d | #2 | Not Started |
| 4 | card-effect-engine | PRD 伪随机分布引擎（5% 步进 + 怜悯保护） | `card-effect-engine/story-004-*.md` | Logic | 0.5d | #1 | Not Started |
| 5 | card-effect-engine | AI 干跑评估接口（GameStateSnapshot 不可变纯计算） | `card-effect-engine/story-005-*.md` | Logic | 0.5d | #3, #4 | Not Started |
| 6 | deployment-system | 内部状态机 + 阵位数据管理（STANDBY→READY→ACTED） | `deployment-system/story-001-*.md` | Logic | 0.5d | — | Not Started |
| 7 | deployment-system | deploy / remove / is_targetable 前后排保护 O(1) | `deployment-system/story-002-*.md` | Logic | 0.5d | #6 | Not Started |
| 8 | deployment-system | 战斗结束 serialize_field 快照导出 GSM.battle.deployment_snapshot | `deployment-system/story-003-*.md` | Integration | 0.5d | #7 | Not Started |
| 9 | deployment-system | clear_standby_state + mark_unavailable + revive_character | `deployment-system/story-004-*.md` | Logic | 0.5d | #7 | Not Started |
| 10 | binding-system | BindingRecord RefCounted 实例模型 + 内部注册表 | `binding-system/story-001-*.md` | Logic | 0.5d | — | Not Started |
| 11 | binding-system | bind / unbind / get_bindings 查询 API | `binding-system/story-002-*.md` | Logic | 0.5d | #10 | Not Started |
| 12 | binding-system | 绑定生命周期信号总线（7 个 Cat 2b 信号） | `binding-system/story-003-*.md` | Integration | 0.5d | #11 | Not Started |
| 13 | binding-system | serialize_all 快照导出 + persistent effect 接口 | `binding-system/story-004-*.md` | Integration | 0.5d | #11 | Not Started |
| 14 | formation-system | 内部条件状态机 + 阵法位管理 | `formation-system/story-001-*.md` | Logic | 0.5d | — | Not Started |
| 15 | formation-system | 激活条件实时重判（订阅 deployment 信号） | `formation-system/story-002-*.md` | Integration | 0.5d | #14, #7 | Not Started |
| 16 | formation-system | get_aura_bonus O(1) 查询 + 梯度光环计算 | `formation-system/story-003-*.md` | Logic | 0.5d | #14 | Not Started |
| 17 | formation-system | serialize_all 快照导出 GSM.battle.formation_snapshot | `formation-system/story-004-*.md` | Integration | 0.5d | #16 | Not Started |
| 18 | ai-system | EnemyTemplate Resource + EnemyFactory + EnemyBattleState | `ai-system/story-001-*.md` | Logic | 0.5d | #0 | Not Started |
| 19 | ai-system | execute_turn 决策主循环（普通/精英/Boss 分支） | `ai-system/story-002-*.md` | Logic | 0.5d | #18, #5 | Not Started |
| 20 | ai-system | BossPhaseMgr 阶段转换内部状态机 | `ai-system/story-003-*.md` | Logic | 0.5d | #18 | Not Started |
| 21 | ai-system | 难度缩放 + register_preconfigured_bindings | `ai-system/story-004-*.md` | Logic | 0.5d | #19, #20 | Not Started |
| 22 | combat-system | 7 阶段回合状态机（advance_phase 确定性推进 + 阶段转换校验） | `combat-system/story-001-*.md` | Logic | 1.5d | #5, #7, #13, #17, #21 | Not Started |
| 23 | combat-system | 战斗生命周期编排（battle_start / battle_end + GSM battle.* 域） | `combat-system/story-002-*.md` | Integration | 0.5d | #22 | Not Started |
| 24 | combat-system | play_card 出牌 + 目标解析 + 自动推进调度 | `combat-system/story-003-*.md` | Logic | 0.5d | #22 | Not Started |
| 25 | combat-system | 阶段转换 Cat 2b 信号通知 CombatUI | `combat-system/story-004-*.md` | Integration | 0.5d | #23 | Not Started |
