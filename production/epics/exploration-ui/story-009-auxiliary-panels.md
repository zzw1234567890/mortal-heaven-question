...
- [ ] 地图状态面板「关闭」→ 返回节点图
- [ ] 卡组查看打开 → 当前卡组网格展示（X/30 张计数）
- [ ] 点击卡牌 → 卡牌详情弹窗
- [ ] 筛选标签（全部/角色/功法/…）→ 列表按类型筛选；排序（费用/稀有度/类型）切换
- [ ] 探索中不可编辑卡组（无编辑入口——仅查看）
- [ ] 卡组查看关闭后返回节点图
...
- 卡组编辑 UI epic: 编辑功能（本 story 明确只读）
- 探索系统域: 解锁判定与统计数据源
...
- **AC-3**: 卡组查看
  - Setup: 28 张卡组的探索存档
  - Verify: 计数 28/30、筛选（按类型）、排序（费用/稀有度）、卡牌详情、无编辑入口
  - Pass condition: 筛选排序结果正确 + 只读验证（无任何写入口）+ 签批
