...
### 缺口 #2（已解决，game-designer 裁决 2026-08-16）：PRD 累加常数 C 未校准（card-effect-engine Story 4-4）

GDD §调优参数 + 待解决问题 #4 原写「PRD 累加常数 C 待游戏测试校准，安全范围 0.3-1.0」。实现 Story 4-4 时发现原公式 `P_start = P_base` + 失败累加 `P_base × C` 结构上无法收敛到标示值（实测 C=0.3 长期触发率 40%）。game-designer 裁决：**改为标准 PRD**——起始值 = 校准常数 C（恒 < p），失败累加 C（单一常数），触发重置 C，runtime `calibrate_C(p)` 二分求解使含怜悯截断的期望触发次数 E[N] = 1/p（30% → C≈0.108）。已同步 GDD（§9 机制 + 调优表 + 待解决问题 #4）+ ADR-0009 §PRD 伪随机分布引擎。AC-002 的 [24, 36] 区间在修正公式 + seed=42 下实测 31 次，通过。
