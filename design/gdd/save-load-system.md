...
{
  "schema_version": 1,                   # 递增整数——**唯一**的迁移驱动字段（见 ADR-0003）
  "version": "1.0.0",                    # 语义化版本——**仅用于面向用户展示**，不参与逻辑
  ...
  "complete": true                       # 完整性标记（纵深防御）
}

> **版本字段说明**：`schema_version`（整数）由 ADR-0003 的迁移链使用——每次数据格式变更时递增。`version`（字符串）仅为存档列表 UI 展示，不参与任何 `if` 判断。详见 `docs/decisions/ADR-0003-save-load-system-json-migration-chain.md`。
...
> **ADR-0003 细化**：上述版本号 (`version`) 仅用于面向用户的展示和兼容性**展示**判断。实际的迁移逻辑由递增整数 `schema_version` 驱动——每次数据格式变更（新增必填字段、键重命名、结构变更）时 +1。存档容器同时包含两个字段：`"schema_version": 1`（迁移驱动）和 `"version": "1.0.0"`（展示）。详见 ADR-0003。
...
- **存档写入中断（游戏崩溃）**：JSON写入非原子操作→写入中崩溃可能产生不完整文件→通过原子 rename 策略（先写 `.tmp` 再 rename）保证写入原子性；辅以 `"complete": true` 标记作为纵深防御。详见 ADR-0003
