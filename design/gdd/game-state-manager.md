...
add_cultivation(amount: int)                    # 修为增加（含上限校验）
spend_resource(type: Enum, amount: int) → bool  # 资源消费（含余额校验，失败返回false）
add_card_to_collection(card_id: String)         # 添加卡牌收藏
...
```
max_cultivation(realm) = BASE_MAX × 1.5^(realm_level - 1)
```

| 变量 | 类型 | 范围 | 描述 |
|------|------|------|------|
| realm_level | int | [1,4] | 1=炼气, 2=筑基, 3=金丹, 4=元婴 |
| BASE_MAX | int | 固定值 | 炼气期修为上限（待数值设计确定） |
| max_cultivation | int | [BASE_MAX, BASE_MAX×3.375] | 当前境界修为上限 |

**验证：** 炼气(L1) → BASE_MAX；筑基(L2) → BASE_MAX×1.5；金丹(L3) → BASE_MAX×2.25；元婴(L4) → BASE_MAX×3.375
...
- **GSM加载时卡牌模板库尚未就绪**：GSM在 `ready` 信号之前不执行任何带卡牌校验的写入（`add_card_to_collection` 等）。卡牌系统加载完成后通过信号通知GSM模板库就绪
...
|------|:-----:|:--------:|---------|---------|
| 修为上限基准值 BASE_MAX | 1000（待确认） | 500-3000 | 炼气期太快突破，体验不到积累 | 炼气期太久卡住，节奏拖沓 |
| 资源最大上限 | 999999 | 99999-9999999 | 限制成长型资源积累 | 无意义的大数，无负面影响 |
| 战斗中存档限制 | 禁止 | 禁止/允许 | — | 战斗中存档可导致SL（S/L）滥用 |
| 卡组最小张数 | 30 | 20-40 | 策略空间太小 | 卡组太杂凑不齐流派 |
| 卡组最大张数 | 40 | 30-50 | 构筑选择受限 | 卡组稀释，策略精度下降 |
...
| # | 问题 | 影响 | 建议解决时间 |
|---|------|------|------------|
| 1 | `BASE_MAX`（修为上限基准值）的精确数值——需要结合境界系统和渡劫突破系统的设计确定。当前暂设为1000 | 修为成长曲线 | 境界系统设计时 |
| 2 | 卡牌模板库的加载时机——GSM初始化时卡牌模板库是否需要已就绪？还是GSM先以校验跳过模式运行，待模板库加载后再开启校验？ | 启动流程顺序 | 架构阶段决定 |
| 3 | 批量变更信号 `batch_updated` 的载荷格式——是展平所有变更的合并Dictionary还是嵌套的变更列表？ | UI系统实现 | 战斗系统设计时 |
