...
## 下面 @export_enum 字符串必须与 EventEnums.OutcomeType 保持同步（值 0-16，顺序不得重排）。
## 值 0-11 为 ADR-0003 原始 12 值；值 12-16 为 ADR-0009 追加（卡牌效果引擎）——仅完整枚举需同步，事件系统不消费 12-16。
@export_group("结果定义")
@export_enum("添加资源:0", "添加修为:1", "添加卡牌:2", "移除卡牌:3", "治疗:4", "伤害:5", "设置标记:6", "获得天赋:7", "触发战斗:8", "推进章节:9", "恢复行动力:10", "无效果:11", "施加状态:12", "修改属性:13", "触发链:14", "激活阵法:15", "修改费用:16")
var type: int = 11
