...
> **最后更新**: 2026-09-07
> **清单版本**: 2026-09-07
> **覆盖的 ADR**: ADR-0001, ADR-0002, ADR-0003, ADR-0004, ADR-0005, ADR-0006, ADR-0007, ADR-0008, ADR-0009, ADR-0010, ADR-0011, ADR-0012, ADR-0013, ADR-0014, ADR-0015, ADR-0016, ADR-0017, ADR-0018, ADR-0019, ADR-0020, ADR-0021, ADR-0022, ADR-0023, ADR-0024, ADR-0025, ADR-0026, ADR-0027, ADR-0028, ADR-0029, ADR-0030, ADR-0031
...
- **UI 零状态所有权**：全部 UI 系统只读消费游戏状态——数据从 GSM/各系统 API 读取，变更仅通过事件触发对应系统执行；瞬态交互状态与持久设置按 ADR-0031 §2.1 三分类处理 —— 来源: ADR-0031、combat-ui.md / exploration-ui.md 数据需求
- **事件驱动更新**：UI 刷新仅由 Cat 1（GSM 状态信号，如 `batch_updated`、`player_changed`）/Cat 2b（动作通知）信号触发——禁止轮询（输入驱动的纯视觉变换除外） —— 来源: ADR-0007、ADR-0031
- **双焦点双视觉策略**：键盘/手柄焦点环（松石青 2px）与鼠标悬停（墨色边框加粗）独立渲染；两者同时激活时优先显示鼠标悬停态 —— 来源: ADR-0031、combat-ui.md、ADR-0004
- **输入锁栈判定区分设备类型**：UI 交互入口先查 `InputManager` 锁栈公共判定（`is_input_allowed(action_type, device)`，ADR-0004 公共 API）——弹窗打开时底层界面输入冻结 —— 来源: ADR-0004、ADR-0031
...
- **HUD 场景可见性**：探索场景 HUD 可见（节点图侧 AP 强化显示与 HUD 同数据源）；战斗场景全局 HUD 隐藏、combat-ui 接管；HUD 以 CanvasLayer 挂载、AudioManager 节点池挂 SceneManager PersistentLayer（ADR-0031 §1/§1.2），暂停时 PROCESS_MODE_ALWAYS + BGM 总线显式暂停 —— 来源: ADR-0031、边界澄清决策（2026-09-05）
...
- **绝不为表现层系统新增 Autoload**——25 个已超 20 软上限；UI 为场景内节点，HUD/AudioManager 由 SceneManager PersistentLayer/CanvasLayer 挂载（ADR-0031 §1.2）；CI 检查 `project.godot` `[autoload]` 段条目数 == 25 —— 来源: ADR-0031、presentation-layer-risks.md R-08
