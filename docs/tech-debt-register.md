...
- **2026-09-10（TD-010）**（hud Story 002，LP-CODE-REVIEW）：`_tween_fill` 幂等检查仅比较 ratio 不含 show_bar——ratio 相同而 show_bar 翻转时残留 fill tween 未 kill（条已隐藏无视觉错误，仅冗余运行）；`_apply_state` breakthrough_hint text 条件赋值为冗余（常量恒定）——与 TD-007 一并处理 — 从 production/epics/hud/story-002-realm-cultivation-bar.md 追踪
